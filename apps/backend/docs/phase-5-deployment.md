...
4. **docker-compose에 app 추가** — DB+앱을 한 명령으로 통합 실행
...
## Phase 5-4. docker-compose에 app 추가 — 한 명령으로 전부

[docker-compose.yml](../docker-compose.yml)에 `app` 서비스를 추가해서, **DB + 앱을 한 번에** 띄웁니다.
...
[2] mysql 컨테이너 시작 → healthcheck 통과 대기
   ↓
[3] mysql이 healthy 되면 app 컨테이너 시작
   ↓
[4] app: npx prisma migrate deploy  (DB 스키마 최신화)
   ↓
[5] app: node dist/src/main.js      (서버 시작, pino 로그)
   ↓
